...
static NSString *temp_databasePath = @"";

@implementation MouveDB

+ (instancetype)sharedDB{
    static MouveDB *_sharedDB = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedDB = [[MouveDB alloc] init];
        
        temp_databasePath=[self copyDataBaseFile];
        _sharedDB.db = [FMDatabase databaseWithPath:temp_databasePath];
        _sharedDB.pool=[FMDatabasePool databasePoolWithPath:temp_databasePath];
        _sharedDB.pool.delegate=self;
        [_sharedDB.db open];
        [_sharedDB.db setShouldCacheStatements:YES];
...
+(NSString *)copyDataBaseFile{
    ...
    NSString *filePath = [documentsDirectory stringByAppendingPathComponent:@"Mouve.sqlite"];
    
    NSFileManager *fileManager = [NSFileManager defaultManager];
    
    if (![fileManager fileExistsAtPath:filePath]) {
        //Remove the previous file if present in the app's Doc dir
        BOOL status=[fileManager removeItemAtPath:filePath error:nil];
        if (!status) {
            NSLog(@"__Error:Removing file,Doesn't exist...");
        }
        
        NSLog(@"__Copyig the Mouve from mainBundle...");
        NSString *plistPathInBundle = [[NSBundle mainBundle] pathForResource:@"Mouve" ofType:@"sqlite"];
        [fileManager copyItemAtPath:plistPathInBundle toPath:filePath error:nil];
    }
    return filePath;
}
